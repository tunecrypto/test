...
const char * const BITCOIN_CONF_FILENAME = "tune.conf";
const char * const BITCOIN_PID_FILENAME = "tuned.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "tune" is a composite category enabling all Tune-related debug output
            if(ptrCategory->count(string("tune"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TuneCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TuneCore
    // Mac: ~/Library/Application Support/TuneCore
    // Unix: ~/.tunecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TuneCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TuneCore";
#else
    // Unix
    return pathRet / ".tunecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty tune.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override tune.conf
