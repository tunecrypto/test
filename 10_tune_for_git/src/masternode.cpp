...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
	...
	int MASTERNODE_PRICE = 1000 + floor(chainActive.Height() / 10000) * 500 ;
        if(coins.vout[vin.prevout.n].nValue !=MASTERNODE_PRICE * COIN) {
    ...
    // should be at least not earlier than block when 1000 TUN tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 TUN tx -> 1 confirmation
